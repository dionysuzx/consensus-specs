...
The `execution_payload_bid` and `parent_execution_payload` handlers, new in
Gloas, also take a full `BeaconBlock` rather than an isolated operation object.
They process the proposer's signed bid and the parent block's settled execution
payload respectively, both of which are carried by the block in Gloas's
enshrined-PBS flow.
...
boolean field with the verification result. This mocking applies only to the
pre-Gloas `execution_payload` handler; in Gloas, payload verification moves out
of block-processing and is exercised by the
`fork_choice/on_execution_payload_envelope` test type instead.

In Gloas, `process_withdrawals` no longer takes an `ExecutionPayload` argument:
withdrawals are computed entirely from state. Test cases under
`operations/withdrawals/` therefore omit the `execution_payload.ssz_snappy`
input file when the runtime spec is Gloas or later.
